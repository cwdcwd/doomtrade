...
describe("Trade History & Performance Analytics (#24)", () => {
  ...
  describe("GET /api/trades/analytics", () => {
    function createApp(db: Database) {
      ...
      const state: AppState = { decisionStore, tradeEngine, portfolio, config: mockConfig as Config, currentMode: "sim", modeChangedAt: Date.now(), db } as AppState;
      const app = express();
    ...
    it("should return analytics after trades", async () => {
      // Doombot's route counts win/loss from sell trades only (closed positions)
      insertTrade({ side: "sell", status: "filled", realizedPnl: 500, fee: 5 });
      insertTrade({ side: "sell", status: "filled", realizedPnl: -100, fee: 1 });
      ...
      expect(resp.body.analytics.losses).toBe(1);
      expect(resp.body.analytics.totalPnl).toBeCloseTo(400, 1);
    });

    it("should filter analytics by symbol", async () => {
      insertTrade({ symbol: "BTC/USDT", side: "sell", status: "filled", realizedPnl: 500 });
      insertTrade({ symbol: "ETH/USDT", side: "sell", status: "filled", realizedPnl: 200 });
